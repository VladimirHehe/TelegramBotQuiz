...
def send_question(bot, message, current_question):
    ...
    poll = {
        ...
        'type': 'quiz',
        'is_anonymous': False,
        'correct_option_ids': [question_data['correct_answer'] - 1],
        'explanation': question_data['explanation'] if 'explanation' in question_data else None,
        'open_period': 60,
        'close_date': None
    }
    bot.send_poll(chat_id=message.chat.id, **poll, reply_markup=get_poll_keyboard(current_question))

def get_poll_keyboard(current_question):
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("Следующий вопрос", callback_data=f'question_{current_question}'))
    return markup
